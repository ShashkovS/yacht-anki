...
## Summary
Цель фазы 06: перевести приложение с пустого каркаса на реальный встроенный учебный каталог. После этой фазы при любом старте приложения в БД идемпотентно загружаются 3 builtin-колоды и 70 builtin-карточек из файлов репозитория, а UI phase05 начинает работать на штатном контенте без route-моков.
...
Итоговый builtin-пакет: ровно 70 карточек.
