...
## Summary
Цель фазы 06: перевести приложение с пустого каркаса на реальный встроенный учебный каталог. После этой фазы при любом старте приложения в БД идемпотентно загружаются 3 builtin-колоды и 74 builtin-карточки из файлов репозитория, а UI phase05 начинает работать на штатном контенте без route-моков.
...
Поддерживаемые `template_type` в актуальном каталоге:
- `term_definition`
- `directional`
- `trim`
- `concept`
...
- `right-of-way`: 23 карточки
  - правила 10, 11, 12, 13, 14, 16, 18
  - overlap
  - clear ahead / astern
  - mark-room scenes с buoy и zone

Итоговый builtin-пакет: ровно 74 карточки.
